...
class CreateUserViewController: BackViewController {
    
    override init(nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
        super.init(nibName: nibNameOrNil, bundle: nibBundleOrNil)
        self.logicLayer = CreateUserLogicLayer()
    }
    ...
    func setupUI() {
        ...
        }).c_setBorderStyle(.line).c_setTextAlignment(.center).c_setPlaceholderText("Enter domain").c_setBorderColor(Palette.orange).c_setBorderWidth(4).c_setBackgroundColor(Palette.lemon)
        
        (configure(UIButton(), at: view, constraints: {make in
            addMarkerBasic(make)
            make.centerY.equalTo(UIScreen.main.bounds.midY + 60 + 35);
        }).c_setTitle("Save user", for: .normal).c_setBackgroundColor(Palette.lemon).c_setBorderWidth(4).c_setBorderColor(Palette.orange) as UIButton).c_addTarget(logicLayer, action: #selector((logicLayer as! CreateUserLogicLayer).saveCallback), for: .touchUpInside)
    }
}
